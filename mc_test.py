...
def monte_carlo_backtest(
  ...
  # Seed opening cash
  holding[0,-1] = seed_capital #_flt[0]
  ...
  for r in range(positions.shape[0]):
    ...
    if r == 0:
      #prior_pos = None
      prior_pos = np.empty(1, dtype=np.float64)
    else:
    ...
    if len(sample_idx) != 0:
      ...
      for i in sample_idx:
        ...
        #new_hold = np.empty(1, dtype=np.float64) #----------------------------------------------------------------------------------------
        #np.round_(holding[r,i] * cash_avail / max_positions_flt / prices[r,i], decimals=0 , out=new_hold) #-------------------------------
        holding[r,i] = new_hold[0]
    
    # Update cash balance for purchases
    if len(sample_idx) != 0:                                                     ############### NOT PICKING UP FIRST LOOP ###############  
    #if sample_idx is not None:
      cost_purch = 0
      for i in sample_idx:
  ...
  cmgr = 1 #np.power(portfolio_vltn[-1] / portfolio_vltn[0], 1 / (len(portfolio_vltn) - 1)) - 1

  # Annual growth rate
  cagr = 1 #np.power(1 + cmgr, 12) - 1

  # Volatility
  volatility = 1 #np.std(np.ediff1d(np.log(portfolio_vltn))) * np.sqrt(12)
...
def monte_carlo_backtest1(
  ...
  vol = []
  for i in range(iter):
    result = monte_carlo_backtest_np(
      prices=prices, 
      positions=positions, 
